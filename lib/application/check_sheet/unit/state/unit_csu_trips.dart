...
List<Map<String, dynamic>> listToJson(List<CSUTrips> list) {
  return list.map((item) => item.toJson()).toList();
}

List<UnitCSUTrips> listCSUResultTripsFromJson(List<dynamic> jsonList) {
  return jsonList
      .map((e) => UnitCSUTrips.fromJson(e as Map<String, dynamic>))
      .toList();
}

String listCSUResultTripsToJson(List<UnitCSUTrips> resultList) {
...
class UnitCSUTrips with _$UnitCSUTrips {
  const factory UnitCSUTrips(
          {required int idUnit,
          @JsonKey(toJson: listToJson) required List<CSUTrips> csuResult}) =
      _UnitCSUTrips;
