  ...
  Future<List<SPK>> searchSPKList({required String search}) async {
    try {
      ...
      data.addAll({
        "mode": "SELECT",
        "command":
            "SELECT id_spk, spk_no, supir1_nm, supir2_nm, nopol FROM $dbName WHERE dc_sta = 1 AND tiba_sta = 0 AND kmbl_sta = 0 AND id_spk LIKE '%$search%' OR spk_no LIKE '%$search%' OR supir1_nm LIKE '%$search%' OR supir2_nm LIKE '%$search%' OR nopol LIKE '%$search%' ORDER BY spk_tgl DESC OFFSET 0 ROWS FETCH FIRST 100 ROWS ONLY",
      });
  ...
  Future<List<SPK>> getSPKList({required int page}) async {
    try {
      ...
      data.addAll({
        "mode": "SELECT",
        "command":
            "SELECT id_spk, spk_no, supir1_nm, supir2_nm, nopol FROM $dbName WHERE dc_sta = 1 AND tiba_sta = 0 AND kmbl_sta = 0 ORDER BY spk_tgl DESC OFFSET $page ROWS FETCH FIRST 10 ROWS ONLY",
      });
